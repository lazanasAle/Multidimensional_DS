...
vector<T> two_d_range_tree<T, CMP1, CMP2>::two_d_range_search(pair<T, T> &interval) {
	T lower = interval.first;
	T upper = interval.second;
	two_d_range_node_iterator<T, CMP2> left_bound = this->lower_bound(lower);
	two_d_range_node_iterator<T, CMP2> right_bound = this->upper_bound(upper);
	vector<set<T, CMP2> *> secondaries;
	auto next_to_right = right_bound.next().context();
	for (auto &bound = left_bound; comparator(bound.context(), next_to_right) != 0; bound = bound.next())
		secondaries.push_back(bound.secondary());
	vector<set<T, CMP2>> secondaries_left;
	set<T, CMP1> returning_set;
	for (auto &sec_set : secondaries) {
		auto left_y = sec_set->lower_bound(lower);
		auto right_y = sec_set->upper_bound(upper);
		for (auto &it = left_y; it != right_y; ++it) {
			bool cond = comparator(*it, interval.first) >= 0 && comparator(*it, interval.second) <= 0;
			if (cond)
				returning_set.insert(*it); 
		}
	}
	vector<T> ret_v(returning_set.begin(), returning_set.end());
	return ret_v;
}
